...
def processIMU_message(imuMsg):
    ...
    pitch = 0
    yaw = 0
    quaternion = (imuMsg.orientation.x, imuMsg.orientation.y,
                  imuMsg.orientation.z, imuMsg.orientation.w)
    data = quaternion_to_euler(
    ...
    linAccLabel.text = str(round(imuMsg.linear_acceleration.x, precision)) + " / " + str(round(
        imuMsg.linear_acceleration.y, precision)) + " / " + str(round(imuMsg.linear_acceleration.z, precision))
    angVelLabel.text = str(round(imuMsg.angular_velocity.x, precision)) + " / " + str(round(
        imuMsg.angular_velocity.y, precision)) + " / " + str(round(imuMsg.angular_velocity.z, precision))
...
scene.width = 500
scene.height = 500
scene2 = canvas(title='9DOF Razor IMU Roll, Pitch, Yaw', x=550, y=0,
                width=500, height=500, center=vector(0, 0, 0), background=color.black)
scene2.range = 1
scene2.select()
cil_roll = arrow(pos=vector(-0.5, 0.3, 0), axis=vector(0.2,
                                                       0, 0), shaftwidth=0.1, color=color.red)
cil_roll2 = cylinder(pos=vector(-0.5, 0.3, 0),
                     axis=vector(-0.2, 0, 0), radius=0.01, color=color.red)
cil_pitch = arrow(pos=vector(0.5, 0.3, 0), axis=vector(
    0, 0, -0.4), shaftwidth=0.02, color=color.green)
arrow_course = arrow(pos=vector(0.0, -0.4, 0), color=color.cyan,
                     axis=vector(0, 0.2, 0), shaftwidth=0.02)
...
label(pos=vector(0.0, -0.16, 0), text="N",
      box=0, opacity=0, color=color.yellow)
label(pos=vector(0.0, -0.64, 0), text="S",
      box=0, opacity=0, color=color.yellow)
label(pos=vector(-0.24, -0.4, 0), text="W",
      box=0, opacity=0, color=color.yellow)
label(pos=vector(0.24, -0.4, 0), text="E",
      box=0, opacity=0, color=color.yellow)
label(pos=vector(0.18, -0.22, 0), height=7,
      text="NE", box=0, color=color.yellow)
label(pos=vector(-0.18, -0.22, 0), height=7,
      text="NW", box=0, color=color.yellow)
label(pos=vector(0.18, -0.58, 0), height=7,
      text="SE", box=0, color=color.yellow)
label(pos=vector(-0.18, -0.58, 0), height=7,
      text="SW", box=0, color=color.yellow)
rollLabel = label(pos=vector(-0.5, 0.52, 0), text="-",
                  box=0, opacity=0, height=12)
pitchLabel = label(pos=vector(0.5, 0.52, 0), text="-",
                   box=0, opacity=0, height=12)
yawLabel = label(pos=vector(0, -0.06, 0), text="-",
                 box=0, opacity=0, height=12)
label(pos=vector(0, 0.9, 0),
      text="Linear Acceleration x / y / z (m/s^2)", box=0, opacity=0)
label(pos=vector(0, -0.8, 0),
      text="Angular Velocity x / y / z (rad/s)", box=0, opacity=0)
linAccLabel = label(pos=vector(0, 0.82, 0), text="-",
                    box=0, opacity=0, height=12)
angVelLabel = label(pos=vector(0, -0.88, 0), text="-",
                    box=0, opacity=0, height=12)
scene.select()
...
p_line = box(length=1.1, height=0.08, width=0.1, color=color.yellow,
             up=vector(0, 0, 1), axis=vector(-1, 0, 0))
